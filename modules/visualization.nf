...
process TAXONOMIC_BARPLOT {
    ...
    """
    qiime feature-classifier classify-sklearn --i-reads ${ot_qza} --i-classifier ${classifier_qza} --o-classification ${taxonomyq}

    qiime metadata tabulate --m-input-file ${taxonomyq} --o-visualization ${taxonomyv}

    qiime taxa barplot --i-table ${ottab_qza} --i-taxonomy ${taxonomyq} --m-metadata-file ${medata_ch} --o-visualization ${taxabar}
